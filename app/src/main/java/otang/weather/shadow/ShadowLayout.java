	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (child.getVisibility() != View.GONE) {
			...
			if (shadowMeasureWidthMatchParent) {
				...
				maxWidth = Math.max(maxWidth, child.getMeasuredWidth() + shadowMarginLeft + shadowMarginRight
						+ lp.leftMargin + lp.rightMargin);
			}
			if (shadowMeasureHeightMatchParent) {
				maxHeight = Math.max(maxHeight, child.getMeasuredHeight() + lp.topMargin + lp.bottomMargin);
			} else {
				maxHeight = Math.max(maxHeight, child.getMeasuredHeight() + shadowMarginTop + shadowMarginBottom
		...
		setMeasuredDimension(
				View.resolveSizeAndState(maxWidth, shadowMeasureWidthMatchParent ? widthMeasureSpec : widthSpec,
						childState),
				View.resolveSizeAndState(maxHeight, shadowMeasureHeightMatchParent ? heightMeasureSpec : heightSpec,
			...
			if (child.getVisibility() != View.GONE) {
				...
				case Gravity.CENTER_HORIZONTAL:
					childLeft = parentLeft + (parentRight - parentLeft - width) / 2 + lp.leftMargin - lp.rightMargin
							+ shadowMarginLeft - shadowMarginRight;
					break;
				case Gravity.RIGHT:
					if (!forceLeftGravity) {
				...
				case Gravity.CENTER_VERTICAL:
					childTop = parentTop + (parentBottom - parentTop - height) / 2 + lp.topMargin - lp.bottomMargin
							+ shadowMarginTop - shadowMarginBottom;
					break;
				case Gravity.BOTTOM:
	...
	protected void onDraw(Canvas canvas) {
		...
		if (canvas != null) {
			...
			Path path = ShapeUtils.roundedRect(shadowMarginLeft, shadowMarginTop, (w - shadowMarginRight),
					(h - shadowMarginBottom), cornerRadiusTL, cornerRadiusTR, cornerRadiusBR, cornerRadiusBL);
